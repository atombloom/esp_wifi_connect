...
void WifiManager::StartStationWithCredentials(const std::string& ssid,
                                              const std::string& password,
                                              bool enable_scan) {
    // 使用 unique_lock 以便在回调前安全地解锁，避免死锁
    ...
    if (config_mode_active_) {
        ...
        // 在回调前释放互斥锁，防止回调内部再次访问 WifiManager 时死锁
        lock.unlock();
        NotifyEvent(WifiEvent::ConfigModeExit);
        lock.lock();
    }

    ESP_LOGI(TAG, "Starting station with explicit credentials, scan: %s",
             enable_scan ? "enabled" : "disabled");
    ...
    if (enable_scan) {
        ...
        station_->StartWithoutScan(ssid, password);
    }
